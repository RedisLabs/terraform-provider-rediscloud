...
Name an API-to-Framework mapper `<Domain>FromAPI`. Use the natural singular or
plural domain name to describe the returned value. Do not add `List`, `Set`,
`Map`, or `Object` merely to restate the Terraform return type. The function
signature and schema already communicate the representation, and omitting it
keeps the name stable if the representation changes. For example, use
`PricingFromAPI`, `ResourceTagsFromAPI`, `CloudProvidersFromAPI`, and
`cloudRegionsFromAPI`.

When two mappers operate on the same domain, qualify them by consumer or purpose
rather than collection kind.
...
Keep collection terms in custom type and constructor names when the
representation is part of the named type's identity.
...
Share a custom type only when those consumers expose the same Terraform element
shape. Related blocks with different nested fields require different Framework
types. Do not add a field to one schema solely to make its type reusable; treat a
computed-only variant as a computed shape instead.
...
Add a `<TypeName>SchemaMatchesModel` unit test for every migrated resource and
data source. Instantiate the implementation, call its `Schema` method, and
compare the complete schema type with a `types.ObjectType` derived from its
`tfsdk` model through `customtypes.AttrTypesOf`. Seed every list, set, or map
field with a correctly typed empty or null value from its mapper or constructor
because an uninitialised collection cannot report its element type. The complete
comparison must cover every schema name, every `tfsdk` tag, and their scalar and
collection types without requiring a live API.

For every custom nested collection, also compare the custom type's
`ElementType()` with the corresponding schema `NestedObject.Type()`, and repeat
that check recursively for nested custom collections. A block's `Type()` returns
its configured `CustomType` directly, so the complete top-level comparison does
not independently verify the manually declared nested object shape.
...
3. Run `go build ./...`, `go vet ./...`, and `golangci-lint fmt --diff`.
