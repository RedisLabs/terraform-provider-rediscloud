...
* `global_enable_passwordless` - (Optional) When `true`, the database is configured without a password. Only valid when the subscription has `public_endpoint_access` disabled. Cannot be `true` when `global_password` is set. Default: `false`
