...
func TestFilterSubscriptions(t *testing.T) {
	...
	t.Run("single filter narrows the set", func(t *testing.T) {
		got := utils.FilterSubscriptions(subs, []func(*subscriptions.Subscription) bool{isActiveActive})
		assert.Equal(t, []*subscriptions.Subscription{aaAlpha}, got)
	})

	t.Run("filters are ANDed together", func(t *testing.T) {
		got := utils.FilterSubscriptions(subs, []func(*subscriptions.Subscription) bool{isActiveActive, namedAlpha})
		assert.Equal(t, []*subscriptions.Subscription{aaAlpha}, got)
	})

	t.Run("a name filter keeps every match", func(t *testing.T) {
		got := utils.FilterSubscriptions(subs, []func(*subscriptions.Subscription) bool{namedAlpha})
		assert.Equal(t, []*subscriptions.Subscription{aaAlpha, singleAlpha}, got)
	})

	t.Run("no matches returns empty", func(t *testing.T) {
		got := utils.FilterSubscriptions(subs, []func(*subscriptions.Subscription) bool{
