...
func TestAccResourceRedisCloudAclUser_CRUDI(t *testing.T) {
	...
	testCreateTerraform := fmt.Sprintf(pro.testAccResourceRedisCloudProDatabase, exampleCloudAccountName, exampleSubscriptionName, exampleDatabasePassword) +
	...
	testUpdateTerraform := fmt.Sprintf(pro.testAccResourceRedisCloudProDatabase, exampleCloudAccountName, exampleSubscriptionName, exampleDatabasePassword) +
		fmt.Sprintf(referencableRole, exampleRoleNameUpdated) +
		fmt.Sprintf(testUser, testUserName, testUserPassword)

	testNewNameTerraform := fmt.Sprintf(pro.testAccResourceRedisCloudProDatabase, exampleCloudAccountName, exampleSubscriptionName, exampleDatabasePassword) +
		fmt.Sprintf(referencableRole, exampleRoleNameUpdated) +
		fmt.Sprintf(testUser, testUserNameUpdated, testUserPassword)

	testNewPasswordTerraform := fmt.Sprintf(pro.testAccResourceRedisCloudProDatabase, exampleCloudAccountName, exampleSubscriptionName, exampleDatabasePassword) +
		fmt.Sprintf(referencableRole, exampleRoleNameUpdated) +
			...
			// Test user creation
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					func(s *terraform.State) error {
						...
						client := provider.testProvider.Meta().(*utils.ApiClient)
