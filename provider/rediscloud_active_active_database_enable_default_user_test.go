		...
		Steps: []resource.TestStep{
			...
			// Step 2: global=true, us-east-1 explicit false (field SHOULD appear in override_region)
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					// Use TestCheckTypeSet to verify specific TypeSet element fields
					resource.TestCheckTypeSetElemNestedAttrs(databaseResourceName, "override_region.*", map[string]string{
						"name":                "us-east-1",
						"enable_default_user": "false",
					}),
					// us-east-2 should NOT have enable_default_user field in this step
			...
			{
				PreConfig: func() {
					t.Logf("Starting Step 5: global=false, both regions inherit")
				},
				Config: fmt.Sprintf(
			...
			{
				PreConfig: func() {
					t.Logf("Starting Step 6: global=false, us-east-1 explicit false (matches global)")
				},
				Config: fmt.Sprintf(
...
// expected: map[regionName]expectedValue (nil means should match global)
