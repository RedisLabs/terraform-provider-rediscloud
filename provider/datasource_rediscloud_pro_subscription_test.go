...
const (
	proSubscriptionConfigPath                   = "./pro/testdata/testAccDatasourceRedisCloudProSubscription.tf"
	proSubscriptionDataSourceConfigPath         = "./pro/testdata/testAccDatasourceRedisCloudProSubscriptionDataSource.tf"
	AADatabaseProDatasourceAADatabaseConfigPath = "./pro/testdata/testAccDatasourceRedisCloudAADatabaseWithProDataSource.tf"
)
...
func TestAccDataSourceRedisCloudProSubscription_basic(t *testing.T) {
	...
	proSubConfig := readConfigFile(t, proSubscriptionConfigPath)
	proSubConfig = fmt.Sprintf(testCloudAccountName, name)

	proSubDataConfig := readConfigFile(t, proSubscriptionDataSourceConfigPath)
...
func TestAccDataSourceRedisCloudProSubscription_ignoresAA(t *testing.T) {
	...
	config := readConfigFile(t, AADatabaseProDatasourceAADatabaseConfigPath)
	config = fmt.Sprintf(name+"-subscription", name+"-database", password)
...
func readConfigFile(t *testing.T, path string) string {
	...
	if err != nil {
		t.Fatalf("failed to read config file %s: %s", path, err)
	}
	return string(data)
}
