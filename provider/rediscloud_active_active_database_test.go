...
func TestAccResourceRedisCloudActiveActiveDatabase_optionalAttributes(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				ConfigFile: config.StaticFile("./activeactive/testdata/aa_database_optional_attributes.tf"),
				ConfigVariables: config.Variables{
					"subscription_name": config.StringVariable(subscriptionName),
					"database_name":     config.StringVariable(name),
					"password":          config.StringVariable(password),
					"port":              config.IntegerVariable(portNumber),
				},
				Check: resource.ComposeAggregateTestCheckFunc(
