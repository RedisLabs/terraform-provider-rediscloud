...
func TestAccResourceRedisCloudActiveActiveSubscriptionPeering_aws(t *testing.T) {
	...
	overlap, err := provider.cidrRangesOverlap(subCidrRange, cidrRange)
	if err != nil {
	...
	provider.matchesRegex(t, peeringRegion, "^[a-z]+-[a-z]+-\\d+$")

	accountId := os.Getenv("AWS_ACCOUNT_ID")
	provider.matchesRegex(t, accountId, "^\\d+$")

	vpcId := os.Getenv("AWS_VPC_ID")
	provider.matchesRegex(t, vpcId, "^vpc-[a-z\\d]+$")
	...
	resource.ParallelTest(t, resource.TestCase{
		PreCheck: func() {
			...
			provider.testAccAwsPeeringPreCheck(t)
