...
- Run `golangci-lint fmt --diff` to check Go formatting without changing files.
...
If the default caches are not writable, point `GOCACHE` and
`GOLANGCI_LINT_CACHE` at writable directories under `/tmp`. Terraform-based
tests may also need to run outside a restricted command sandbox because they
start a provider process and communicate with it over a local socket.
