...
func TestAccResourceRedisCloudDatabase(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: fmt.Sprintf(testAccResourceRedisCloudDatabase, testCloudAccountName, name, databaseName, password),
			...
			// // TODO: Verify Update test working
			//{
...
const testAccResourceRedisCloudDatabaseReplica = subscriptionBoilerplate + `
resource "rediscloud_database" "replica_of_database" {
    subscription_id = rediscloud_subscription.example.id
    name = "tf-database-3"
    protocol = "redis"
    memory_limit_in_gb = 3
    ...
    throughput_measurement_by = "operations-per-second"
    throughput_measurement_value = 10000  
	replica_of = rediscloud_database.example.
}    
`
